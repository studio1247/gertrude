...
def add_row(condition, row):
    add_cell(condition + " et heures_hebdomadaires<20", row[0])
    add_cell(condition + " et heures_hebdomadaires<25", row[1])
    add_cell(condition + " et heures_hebdomadaires<30", row[2])
    add_cell(condition + " et heures_hebdomadaires<35", row[3])
    add_cell(condition + " et heures_hebdomadaires<40", row[4])
    add_cell(condition, row[5])

def add_table(condition_enfants, seuils, max):
    add_row(condition_enfants + " et revenus<%f" % seuils[0], [max-1.0-0.25*i for i in range(6)])
    add_row(condition_enfants + " et revenus<%f" % seuils[1], [max-0.5-0.25*i for i in range(6)])
...
add_table("enfants=1", (20285.0, 45077.0), 9.0)
