    ...
    def CalculeFraisGardeComplete(self, heures, heures_mois):
        if self.montant_heure_garde is not None:
            ...
            while heure < heures_mois:
                montant, unite = database.creche.eval_tarif(self.debut, self.mode_garde, self.inscrit.handicap, self.assiette_annuelle, self.enfants_a_charge, self.jours_semaine, self.heures_semaine, self.inscription.reservataire, self.inscrit.nom.lower(), self.parents, self.chomage, self.conge_parental, self.heures_mois, heure, self.tranche_paje, self.inscrit.famille.tarifs | self.inscription.tarifs, self.inscription.site.nom if self.inscription.site else "")
    ...
    def __init__(self, inscrit, date, options=0):
        ...
        if database.creche.mode_facturation == FACTURATION_FORFAIT_MENSUEL:
            ...
            try:
                ...
                self.tarif_montant, self.tarif_unite = database.creche.eval_tarif(self.debut, self.mode_garde, inscrit.handicap, self.assiette_annuelle, self.enfants_a_charge, self.jours_semaine, self.heures_semaine, self.inscription.reservataire, inscrit.nom.lower(), self.parents, self.chomage, self.conge_parental, self.heures_mois, 0, self.tranche_paje, inscrit.famille.tarifs | self.inscription.tarifs, self.inscription.site.nom if self.inscription.site else "")
            ...
            try:
                self.tarif_montant, self.tarif_unite = database.creche.eval_tarif(self.debut, self.mode_garde, inscrit.handicap, self.assiette_annuelle, self.enfants_a_charge, self.jours_semaine, self.heures_semaine, self.inscription.reservataire, inscrit.nom.lower(), self.parents, self.chomage, self.conge_parental, self.heures_mois, None, self.tranche_paje, inscrit.famille.tarifs | self.inscription.tarifs, self.inscription.site.nom if self.inscription.site else "")
                if self.tarif_unite == TARIF_HORAIRE_UNITE_EUROS_PAR_HEURE:
                    self.montant_heure_garde = self.tarif_montant
                    if options & TRACES:
